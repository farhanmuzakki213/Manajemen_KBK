...
@section('admin')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <!-- Page Heading -->
                <h5 class="card-title fw-semibold mb-4">Edit Data Matkul KBK </h5>
                <div class="container-fluid">
                    <!-- edit Tambah Data -->
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ route('matkul_kbk.update',['id' => $data_matkul_kbk->id_matkul_kbk]) }}">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                ...
                                {{-- <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="id_matkul_kbk" name="id_matkul_kbk" value="{{$data_matkul_kbk->id_matkul_kbk}}">
                                    @error('id_matkul_kbk')
                                        <small>{{ $message }}</small>
                                    @enderror
                                </div> --}}
                                ...
                                <div class="mb-3">
                                    <input type="hidden" class="form-control" id="id_matkul_kbk" name="id_matkul_kbk" value="{{ $data_matkul_kbk->id_matkul_kbk }}" readonly>
                                    @error('id_matkul_kbk')
                                        <small>{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="nama_matkul" id="nama_matkul" required>
                                        <option disabled>Pilih nama matkul</option>
                                        @foreach ($data_matkul as $matkul)
                                            @php
                                                $isDisabled = \App\Models\MatkulKBK::where('matkul_id', $matkul->id_matkul)->exists();
                                                $isSelected = ($matkul->id_matkul == $data_matkul_kbk->matkul_id) ? 'selected' : '';
                                            @endphp
                                            @unless ($isDisabled && $matkul->id_matkul != $data_matkul_kbk->matkul_id)
                                                <option value="{{ $matkul->id_matkul }}" {{ $isSelected }}>{{ $matkul->nama_matkul }}</option>
                                            @endunless
                                        @endforeach
                                    </select>
                                    @error('nama_matkul')
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="nama_matkul"
                                        ...
                                        @foreach ($data_matkul as $matkul)
                                            <option value="{{ $matkul->id_matkul }}"
                                                {{ $matkul->id_matkul == $data_matkul_kbk->matkul_id ? 'selected' : '' }}>
                                                {{ $matkul->nama_matkul }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="jenis_kbk"
                                        ...
                                        @foreach ($data_jenis_kbk as $jenis_kbk)
                                            <option value="{{ $jenis_kbk->id_jenis_kbk }}"
                                                {{ $jenis_kbk->id_jenis_kbk == $data_matkul_kbk->jenis_kbk_id ? 'selected' : '' }}>
                                                {{ $jenis_kbk->jenis_kbk }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="kurikulum"
                                        ...
                                        @foreach ($data_kurikulum as $data)
                                            <option value="{{ $data->id_kurikulum }}"
                                                {{ $data->id_kurikulum == $data_matkul_kbk->kurikulum_id ? 'selected' : ''}}>
                                                {{ $data->nama_kurikulum }}
                                            </option>
                                        @endforeach
                                    </select>
